...
**Direction approved (Flutter only):** 2026-09-18 · **Dark-only** · Canonical design for `flutter-app/`: root `DESIGN.md` · Prototypes + IA: `docs/design/s12-renewal/` — **does not apply to the RN client**, see the decision below.
...
**Decision 2026-09-18 — client rebuilt in React Native (from scratch).** Vertiege's client is being rebuilt **from scratch** in a new `expo-app/` folder using **React Native + Expo + TypeScript** (Expo Router, NativeWind v4, Reanimated). Visual design is also from scratch — the Renewal design docs and `DESIGN.md` are exploration history for the RN client, **not** applied. Supabase + Firebase backends are reused unchanged; the **only** things carried over from Flutter are **keys and logins** (`.env`, Firebase config, OAuth redirect, app ids). No Flutter code is ported. Stack additions per the 2026 RN guidance (Simon Grimm): Zustand + TanStack Query, MMKV, expo-sqlite + Drizzle, React Hook Form + Zod, FlashList, EAS, Sentry, RevenueCat, PostHog; **Supabase Auth is kept** (instead of Clerk) for backend/RLS continuity. The Flutter app keeps shipping the closed beta until the new app passes both stores, then it is archived as `legacy-flutter/`. Master plan: [`expo-app/rn-rewrite-plan.md`](expo-app/rn-rewrite-plan.md). Flutter tasks S12.4–S12.19 below are **superseded** — they remain as design-history reference only.

**Design decision 2026-09-25 — RN visual language: Neobrutalism (neobrutalism.dev, `yellow` palette, light-only).** Source of truth is the [neobrutalism.dev](https://www.neobrutalism.dev/docs/installation) styling registry + component variant API. Tokens: `--background #FDF7C4` (tinted ground), `--secondary-background #FFFFFF` (cards), `--main #FACC00` (gold — continuity with shipped `#C9A227`), ink/border/ring `#000000`, `--shadow: 4px 4px 0 0 var(--border)` (zero blur), `radius-base 5px`, `font-weight-base 500` / `heading 700`; Vertiege additions: violet `#432DD7` (links/mentions), success/warning/danger, `--background-raised #FFF6D6`. **Light-only** — upstream ships no dark mode, so `ThemeToggle`/`useColorScheme`/`NAV_THEME` are deleted and no `dark:` variants are allowed. Type: Space Grotesk 700 (display) + Plus Jakarta Sans 500 (body). Motion ceiling 150ms / 80ms press. **NativeWindUI is dropped as the UI kit** — upstream `.tsx` files are React DOM + `@base-ui/react` and won't compile in Expo, so `expo-app/components/ui/*.tsx` is a **hand-port** keeping the same tokens, variant names and class semantics (hover→press, `focus-visible:ring`→`borderWidth` 2→3, `shadow-shadow`→RN `boxShadow`); native `lucide-react-native` for icons. Full tokens, translation table, page inventory and phase plan: [`expo-app/rn-rewrite-plan.md` §7–§10](expo-app/rn-rewrite-plan.md).

**Status 2026-09-25 — RN rewrite: P0 (design identity) ✅, P1 (primitive kit + shell) ✅; next is P2 (data layer).** Repo restructure committed as `c345539`. P0 delivered `lib/theme/tokens.ts` + generated `tailwind.tokens.json` (`npm run tokens` / `tokens:check`), `docs/design.md`, NativeWindUI removal, dark-mode removal, and `test/tokens.test.ts`. P1 delivered the 23-component neobrutalism kit in `expo-app/components/ui/`, the `/design` dev showcase route, the `you` → `identity` tab rename, the three transition families (tabs / push / sheet), the pure `lib/router/resolve-redirect.ts` port of GoRouter's redirect chain, and jest infrastructure (`test/setup.ts` + mocks). Gate: `npm run check` green — **7 suites / 107 tests**, plus a green `expo export --platform ios`. Carry-over notes for the next session are in [`expo-app/rn-rewrite-plan.md`](expo-app/rn-rewrite-plan.md) §Status.
