    ...
    if (raw != null) {
      try {
        ...
        if (data['dateKey'] == dateKey) {
          final quests = (data['quests'] as List).cast<Map<String, dynamic>>().map((q) {
  ...
  void claimQuest(String questId) {
    final quests = state.quests.map((q) {
      if (q.id == questId && q.isComplete && !q.claimed) {
        return Quest(
          id: q.id, label: q.label, icon: q.icon, target: q.target,
          progress: q.progress, claimed: true,
        );
      }
      return q;
    }).toList();
  ...
  void _increment(String questId) {
    final quests = state.quests.map((q) {
      if (q.id == questId && !q.isComplete) {
        return Quest(
          id: q.id, label: q.label, icon: q.icon, target: q.target,
          progress: q.progress + 1, claimed: q.claimed,
        );
      }
      return q;
    }).toList();
