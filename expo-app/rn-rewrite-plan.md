...
- Fresh folder `expo-app/` with a brand-new Expo project (scaffolded: `rn-new --nativewindui`, i.e. Expo Router + NativeWind + NativeWindUI + Reanimated on Expo SDK 56 / RN 0.85).
...
| Supabase anon key | from Flutter `.env` (public, RLS-protected) | `expo-app/.env` → `EXPO_PUBLIC_SUPABASE_ANON_KEY` ✅ |
| App identity | name Vertiege, scheme `vertiege`, android `com.vertiege`, iOS `com.vertiege` | `expo-app/app.json` ✅ |
| OAuth redirect | `vertiege://auth/callback` | Supabase Auth config (unchanged) + Expo scheme |
| Firebase project | `veritage` · sender `92526224561` | `expo-app/config/firebase.ts` (when Firebase services land) |
| Firebase Android | apiKey `AIzaSyBZTPAlGRd_kMw743Fe0qJkVHNeeJySU80`, appId `1:92526224561:android:7b7a9f6f448f61ca7cae90` | `expo-app/config/firebase.ts` |
| Firebase iOS | apiKey `AIzaSyDlrYB-KZskQuktszLXHMZ5NGLpp2fJQIY`, appId `1:92526224561:ios:29b2459028dca5757cae90` | `expo-app/config/firebase.ts` |
...
`expo-app/.env` is gitignored; `EXPO_PUBLIC_*` values are build-time embedded (anon key is public by design — security is RLS).
...
| Phase | Scope | Exit |
|-------|-------|------|
| **F0 — Foundation** ✅ | Scaffold `expo-app/` ✅, NativeWind+UI+Reanimated ✅, app identity ✅, keys in `.env` ✅, Supabase client + secure-store session ✅, Zustand + TanStack Query + MMKV + sqlite/Drizzle wiring ✅, RHF + Zod ✅, jest-expo ✅, auth boot — **email only** | App boots, sign-in loop works, `tsc --noEmit` + eslint + jest clean ✅ |
...
**F0 landed 2026-09-25.** Entry points (inside `expo-app/`): `lib/query.ts` (QueryClient), `lib/storage.ts` (MMKV — SecureStore stays for auth tokens), `db/` (Drizzle + expo-sqlite; edit `db/schema.ts` → `npm run db:generate`), `lib/validation/` (Zod), `test/`. Cold-start `/` now routes by auth state, and a corrupt stored session no longer strands the boot spinner.
...
| Secrets in client | Security gate script for `expo-app/` (scan for `service_role`) |
| Expo Go can't run MMKV/dev-client features | dev-client is the single dev target from the start |

## 9. Conventions (RN side)

- Clean Expo structure (inside `expo-app/`): `app/` routes, `components/` UI, `lib/` services/state, `db/` Drizzle schema.
