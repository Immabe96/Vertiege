...
# Usage: .\flutter-app\scripts\image_gen.ps1 next | status | mark | promote | init
param(
...
$RepoRoot = Split-Path $Root -Parent   # repo root — docs/ lives above flutter-app/

$ManifestPath = Join-Path $RepoRoot 'docs\assets\image-manifest.json'
$StagingDir = Join-Path $Root 'assets\staging\generated'
$FinalDir = Join-Path $Root 'assets\generated'
$LogPath = Join-Path $RepoRoot 'docs\assets\generation-log.md'
...
"@
    ...
    'export-chatgpt' {
        ...
        $outPath = Join-Path $RepoRoot 'docs\assets\chatgpt-pending-prompts.md'
    ...
    'export-chatgpt-project' {
        ...
        $outPath = Join-Path $RepoRoot 'docs\assets\chatgpt-project-batch.md'
        $sb = [System.Text.StringBuilder]::new()
