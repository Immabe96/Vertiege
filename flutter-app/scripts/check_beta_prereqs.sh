...
REPO_ROOT="$(cd "$ROOT/.." && pwd)"        # repo root (docs/, supabase/ live there)
cd "$ROOT"
...
if [[ -f ios/Runner/GoogleService-Info.plist ]]; then
  ...
  ok "iOS Firebase in lib/firebase_options.dart (run ./flutter-app/scripts/firebase_beta_sync.sh for plist)"
else
  warn "ios/Runner/GoogleService-Info.plist missing — run ./flutter-app/scripts/firebase_beta_sync.sh after firebase login"
fi
...
if python3 scripts/check_core_achievement_assets.py >/dev/null 2>&1; then
  ...
  warn "Core achievement assets incomplete — run ./flutter-app/scripts/check_core_achievement_assets.py"
fi

# Publish readiness (code present; operator still deploys / hosts)
if [[ -f "$REPO_ROOT/supabase/functions/delete-account/index.ts" ]]; then
  ok "delete-account Edge Function source present"
else
  bad "supabase/functions/delete-account missing (at repo root)"
fi
...
echo "See docs/guides/publish-readiness.md (repo root)"
echo ""
if [[ "$FAIL" -eq 0 ]]; then
