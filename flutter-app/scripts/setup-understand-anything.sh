...
# .cursor/, .understand-anything/ and docs/ all live at the repo root, so
# ROOT here is the repo root — not flutter-app/ (which this script sits under).
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
UA_DIR="${UA_DIR:-$HOME/.understand-anything/repo}"

echo "→ Vertiege repo root: $ROOT"
...
echo "  Restart Cursor, then in Agent chat run: /understand flutter-app/lib"
echo "  (scopes to Dart app code; omit path for full repo scan)"
echo "  Dashboard: /understand-dashboard"
echo ""
echo "  Docs: docs/guides/codebase-map.md"
