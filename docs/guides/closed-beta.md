...
Testers get the **same backend** (production Supabase). Bump **`flutter-app/pubspec.yaml` build number (`+N`)** for every build you distribute.
...
firebase login
cd /path/to/Vertiege
./flutter-app/scripts/firebase_beta_sync.sh
```

That script:

- Lists Android / iOS / Web apps in project **`veritage`**
- Downloads `flutter-app/android/app/google-services.json`
- Downloads `flutter-app/ios/Runner/GoogleService-Info.plist`
- Runs `flutterfire configure` for `flutter-app/lib/firebase_options.dart`

Project default is set in [`.firebaserc`](../../flutter-app/.firebaserc). Override with `FIREBASE_PROJECT=other`.

The app already initializes Firebase from **`flutter-app/lib/firebase_options.dart`**; the plist/json files are still recommended for native Google Sign-In and tooling.

### Local sanity check

```bash
./flutter-app/scripts/check_beta_prereqs.sh
```
...
- Single source: **`flutter-app/pubspec.yaml`** `version: 1.x.y-beta.z+N`
- Run `./flutter-app/scripts/sync_app_version.sh` after every version change (release scripts do this automatically).
- Ship the **same `+N`** to Play and TestFlight: `./flutter-app/scripts/build_release_all.sh`
...
Copy [`.env.template`](../../flutter-app/.env.template) → `.env`:
...
| `ANDROID_KEYSTORE_BASE64` | `base64` of `flutter-app/android/upload-keystore.jks` |
...
```bash
./flutter-app/scripts/setup-android-signing.sh
```
...
4. Build an App Bundle:
   ...
   ```bash
   ./flutter-app/scripts/build_release_appbundle.sh
   ```

   Output: `flutter-app/build/app/outputs/bundle/release/app-release.aab`  
   Copy to `flutter-app/releases/` if you version manifests there.
...
7. For each new build: bump `+N` in `flutter-app/pubspec.yaml`, rebuild AAB, upload to the same track.

**Google Sign-In on Play builds:** register **release** SHA-1 in Firebase + Google Cloud OAuth Android client. Get fingerprint:

```bash
keytool -list -v -keystore flutter-app/android/upload-keystore.jks -alias upload
```
...
3. Share `flutter-app/releases/vertiege-*-arm64-release.apk` (Drive, email, etc.).
...
3. Firebase Console → add **iOS app** `com.vertiege` → download **`GoogleService-Info.plist`** → place at:
   ...
   Template: [flutter-app/ios/Runner/GoogleService-Info.plist.example](../../flutter-app/ios/Runner/GoogleService-Info.plist.example).  
...
```bash
# 1. Version
#    Edit flutter-app/pubspec.yaml — bump +N (and beta name if needed: 1.1.0-beta.3+7)
./flutter-app/scripts/sync_app_version.sh
...
# 3. Build
./flutter-app/scripts/build_release_appbundle.sh    # Play
./flutter-app/scripts/build_release_apk.sh --no-bump   # APK / GitHub (if CI not used)
...
Commit `flutter-app/releases/manifest-*.json` when you track local APK manifests.
...
| [flutter-app/releases/README.md](../../flutter-app/releases/README.md) | APK naming & scripts |
...
- [ ] `flutter-app/ios/Runner/GoogleService-Info.plist` present (Firebase iOS app)
...
Android Firebase (`flutter-app/android/app/google-services.json`) is already in the repo for `com.vertiege`.
